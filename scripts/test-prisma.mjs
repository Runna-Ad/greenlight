...
// ── Higgsfield · paso 1: las familias que el equipo usa ahí + la página de cada modelo ──
{
  ...
  eq("estimado: ilimitado / sin dato / costo sin número", [textoCosto(CATALOGO_BASE.seedream.modelos[0].costo, false).es.startsWith("Ilimitado"), textoCosto(null, true).es.startsWith("Costo: sin dato"), textoCosto(CATALOGO_BASE.nanobanana.modelos[0].costo, false).es.startsWith("Consume créditos")].join(), "true,true,true");
