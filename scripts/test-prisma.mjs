...
// ── Higgsfield · paso 1: las familias que el equipo usa ahí + la página de cada modelo ──
{
  ...
  eq("Kling 3.0 (botón): 3–15 s; 720p 2, 1080p 2.5, 4K 6 por segundo", JSON.stringify([kl.porDuracion.length, kl.porDuracion[2], kl.porDuracion[12]]), JSON.stringify([13, { s: 5, p720: 10, p1080: 12.5, p4k: 30 }, { s: 15, p720: 30, p1080: 37.5, p4k: 90 }]));
  ...
  eq("costoCorto y creditosDe", JSON.stringify([costoCorto(CATALOGO_BASE.seedream.modelos[0].costo).es, costoCorto(CATALOGO_BASE.kling.modelos[1].costo).es, creditosDe(null), creditosDe(CATALOGO_BASE.nanobanana.modelos[1].costo)]), JSON.stringify(["ilimitado", "≈12.5 cr", null, 0]));
...
console.log("\n▶ F6b — vigía");
{
  ...
  ok("aplicar modelo existente: se reemplaza y conserva su costo y su página", reemplazo.ok && reemplazo.fila.modelos.length === cbV.kling.modelos.length && reemplazo.fila.modelos[1].etiqueta === "Kling 3.0 (nuevo)" && reemplazo.fila.modelos[1].costo?.creditos_tipicos === 12.5 && reemplazo.fila.modelos[1].costo?.creditos_por_duracion?.length === 13 && !!reemplazo.fila.modelos[1].url);
