...
console.log("\n▶ F1 — Úsalo en… (recomendarModelo)");
{
  ...
  eq("chatgpt imagen nueva sin texto → Sunburst (cuesta lo mismo que Flare: gana la calidad)", recomendarModelo({ ...base, tool: "chatgpt" }).modelo, "gpt-image-2.5-sunburst");
  ...
  eq("kling 5 s sin voz → 3.0 (cuesta lo mismo que Turbo en 1080p)", recomendarModelo({ ...base, tool: "kling", job: "animar_foto", duracion: 5 }).modelo, "kling-3.0");
...
// ── Higgsfield · paso 1: las familias que el equipo usa ahí + la página de cada modelo ──
{
  ...
  ok("pasos Nano Banana: orden con [Imagen N], 4K para impreso, revisar el texto; sin paso de sonido", pNb.includes("([Imagen 1])") && pNb.includes("4K para la final") && pNb.includes("enciende «Ilimitado»") && pNb.includes("letra por letra") && !pNb.includes("Sonido"), pNb);
  ...
  eq("costoMinimo: ilimitado = 0; Seedance = su Mini; DoP = Standard (10, el recomendado)", JSON.stringify([costoMinimo(CATALOGO_BASE, "seedream"), costoMinimo(CATALOGO_BASE, "seedance"), costoMinimo(CATALOGO_BASE, "higgsfield")]), "[0,12.5,10]");
  const sFast = CATALOGO_BASE.seedance.modelos.find((m) => m.id === "seedance-2.0-fast");
  ok("Seedance 2.0 Fast: detrás de Mini (Mini sigue siendo el recomendado), 720p 3.5/s, su página", CATALOGO_BASE.seedance.modelos[0].id === "seedance-2.0-mini" && sFast.rol === "rapido" && JSON.stringify(sFast.costo.porDuracion[1]) === JSON.stringify({ s: 5, p480: 7.5, p720: 17.5, p1080: null, p4k: null }) && sFast.url.endsWith("seedance_2_0_fast"));
  eq("DoP: Standard · Turbo · Lite con su página; Kling Turbo con la suya", JSON.stringify([CATALOGO_BASE.higgsfield.modelos.map((m) => [m.etiqueta, m.url.split("=")[1], m.costo.tipico]), CATALOGO_BASE.kling.modelos[0].url.split("=")[1]]), JSON.stringify([[["Higgsfield DoP Standard", "standard", 10], ["Higgsfield DoP Turbo", "turbo", 7], ["Higgsfield DoP Lite", "lite", 5]], "kling3_0_turbo"]));
  ...
  eq("Kling corto sin voz → 3.0 (Pedro: al mismo precio, el mejor)", recomendarModelo({ job: "animar_foto", tool: "kling", destino: "tiktok", refs: 1, texto: false, dialogo: false, duracion: 5 }).modelo, "kling-3.0");
  const kCaro = structuredClone(CATALOGO_BASE);
  kCaro.kling.modelos = kCaro.kling.modelos.map((m) => (m.id === "kling-3.0" ? { ...m, costo: { ...m.costo, tipico: 11 } } : m));
  eq("…pero si el fino cuesta MÁS, se queda Turbo", recomendarModelo({ job: "animar_foto", tool: "kling", destino: "tiktok", refs: 1, texto: false, dialogo: false, duracion: 5 }, kCaro).modelo, "kling-3.0-turbo");
  const ct = textoCosto(CATALOGO_BASE.seedance.modelos.find((m) => m.id === "seedance-2.0").costo, true).es;
  ...
  eq("Omni Flash 1.1 (botón): 3–10 s; 720p 3/s, 1080p 4.5/s (3 s = 13.5, el botón muestra 14), 4K 9/s", JSON.stringify([omni.porDuracion.map((f) => f.s).join(), omni.porDuracion[0], omni.porDuracion[7], omni.porDuracion[5].p1080]), JSON.stringify(["3,4,5,6,7,8,9,10", { s: 3, p720: 9, p1080: 13.5, p4k: 27 }, { s: 10, p720: 30, p1080: 45, p4k: 90 }, 36]));
  ...
  ok("estimado exacto a la duración: Omni 5 s = 22.5 en 1080p, 720p 15, 4K 45", cOmni.startsWith("22.5 créditos por intento a 5 s en 1080p · 720p: 15 · 4K: 45 · US$0.90.") && cOmni.includes("≈ 67.5–112.5"), cOmni);
  const kl = CATALOGO_BASE.kling.modelos.find((m) => m.id === "kling-3.0").costo;
  eq("Kling 3.0 (cuenta de Rünna): 3–15 s; 720p 1.75, 1080p 2, 4K 6 por segundo", JSON.stringify([kl.porDuracion.length, kl.porDuracion[2], kl.porDuracion[12]]), JSON.stringify([13, { s: 5, p720: 8.75, p1080: 10, p4k: 30 }, { s: 15, p720: 26.25, p1080: 30, p4k: 90 }]));
  const kt = CATALOGO_BASE.kling.modelos.find((m) => m.id === "kling-3.0-turbo").costo;
  eq("Kling 3.0 Turbo: 720p 1.5, 1080p 2, sin 4K", JSON.stringify([kt.tipico, kt.porDuracion[2]]), JSON.stringify([10, { s: 5, p720: 7.5, p1080: 10, p4k: null }]));
  ...
  ok("duración sin fila en la tabla → el típico (no se inventa)", textoCosto(omni, true, "gemini_omni", 12).es.startsWith("≈ 36 créditos por intento") && textoCosto(omni, false, "gemini_omni", 5).es.startsWith("≈ 36"));
  ...
  eq("Seedance 2.0 (botón): 4–15 s; 480p 3, 720p 4.5, 1080p 9, 4K 22 por segundo", JSON.stringify([s20.porDuracion.length, s20.porDuracion[0], s20.porDuracion[11]]), JSON.stringify([12, { s: 4, p480: 12, p720: 18, p1080: 36, p4k: 88 }, { s: 15, p480: 45, p720: 67.5, p1080: 135, p4k: 330 }]));
  ok("Seedance sin duración en la tabla: nota propia (por segundo), no la general de ×2", textoCosto(s20, true, "seedance", null).es.includes("9 créditos por segundo") && !textoCosto(s20, true, "seedance", null).es.includes("doble"));
  ...
  eq("Seedance Mini: 4–15 s, 480p 1/s y 720p 2.5/s: 4 s = 10, 15 s = 37.5 (Higgsfield muestra 38)", JSON.stringify([sMini.porDuracion.length, sMini.porDuracion[0], sMini.porDuracion[11].p720, sMini.max]), JSON.stringify([12, { s: 4, p480: 4, p720: 10, p1080: null, p4k: null }, 37.5, 37.5]));
  const cMini = textoCosto(sMini, true, "seedance", 6).es;
  ok("estimado Mini a 6 s: 15 en 720p (su máximo), sin 1080p ni 4K", cMini.startsWith("15 créditos por intento a 6 s en 720p (su máximo) · 480p: 6 · US$0.60.") && !cMini.includes("1080p") && !cMini.includes("4K"), cMini);
  ...
  eq("costoCorto y creditosDe", JSON.stringify([costoCorto(CATALOGO_BASE.seedream.modelos[0].costo).es, costoCorto(CATALOGO_BASE.kling.modelos[1].costo).es, creditosDe(null), creditosDe(CATALOGO_BASE.nanobanana.modelos[1].costo)]), JSON.stringify(["ilimitado", "≈10 cr", null, 0]));
...
console.log("\n▶ F6b — vigía");
{
  ...
  ok("aplicar modelo existente: se reemplaza y conserva su costo y su página", reemplazo.ok && reemplazo.fila.modelos.length === cbV.kling.modelos.length && reemplazo.fila.modelos[1].etiqueta === "Kling 3.0 (nuevo)" && reemplazo.fila.modelos[1].costo?.creditos_tipicos === 10 && reemplazo.fila.modelos[1].costo?.creditos_por_duracion?.length === 13 && !!reemplazo.fila.modelos[1].url);
