...
// ── Higgsfield · paso 1: las familias que el equipo usa ahí + la página de cada modelo ──
{
  ...
  eq("Seedance: la duración es la más cercana que ofrece (8 → 10)", /OUTPUT: \S+, (\d+) seconds/.exec(compilar(spec("texto_a_video", "seedance", { duracion: 8 })).texto)?.[1], "10");
  ...
  ok("pasos Seedance: abrir el modelo, refs en orden con @imageN, 10 s · 9:16, 720p→1080p, sonido, subir resultado", pSeed.startsWith("Abre Seedance 2.0 Mini en Higgsfield") && pSeed.includes("1) La persona (@image1) · 2) Referencia de estilo (@image2)") && pSeed.includes("Ajustes: 10 s · 9:16 · 720p para probar") && pSeed.includes("Sonido:") && pSeed.endsWith("H.Ü.E lo revisa y cuenta la vuelta."), pSeed);
